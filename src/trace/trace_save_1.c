...
static void pm_tp_rx(void *data, struct sk_buff *skb);

// static void pm_tp_rx(void *data,
//                      struct sock *sk,
//                      struct msghdr *msg,
//                      size_t        size,
//                      int           flags,
...
static int pm_register_tracepoints(void)
{
        ...
        tracepoint_probe_register(tp_rx, pm_tp_rx, NULL);
        return 0;
}
...
static void pm_unregister_tracepoints(void)
{
        ...
        if (tp_rx)
                tracepoint_probe_unregister(tp_rx, pm_tp_rx, NULL);
        tracepoint_synchronize_unregister();
}

static void pm_tp_tx(void *data, struct sk_buff *skb)
{
		//atomic64_inc(&trace_calls);
...
//                      int           retval)
// {
//         /* Only successful receives are interesting.  retval == number of bytes
//          * copied; 0 → orderly shutdown, <0 → error. */
//         if (retval <= 0)
//                 return;

//         rcu_read_lock();                     /* rhashtable_lookup_fast() rule */
//         {
//                 u32 pid = task_pid_nr(current);
//                 struct pid_metrics *pm =
...
static void pm_tp_rx(void *data, struct sk_buff *skb)
{
        ...
        owner = sk->sk_socket->file->f_owner.pid;
        if (!owner){
            return;
		}
