...
static struct dentry *pm_create_buf_file(const char *filename,
                                        struct dentry *parent,
                                        umode_t mode,
                                        struct rchan_buf *buf,
                                        int *is_global)
{
    /* let relayfs use its stock file_ops */
    extern const struct file_operations relay_file_operations;
    return debugfs_create_file(filename, mode, parent,
                               buf, &relay_file_operations);
}

static int pm_remove_buf_file(struct dentry *dentry)
{
...
static const struct rchan_callbacks relay_cb = {
    ...
    .create_buf_file  = pm_create_buf_file,
    .remove_buf_file  = pm_remove_buf_file,
};
...
/* We have to use two different types of techniques to get the network data
	we need to use tracepoints for TX and kretprobes for RX.
*/
...
static int kp_sock_recvmsg_ret(struct kretprobe_instance *ri, struct pt_regs *regs)
{
	...
	rcu_read_lock();
	{
		struct pid_metrics *pm = rhashtable_lookup_fast(&pid_ht, &pid, ht_params);
...
static void pm_free_rcu(struct rcu_head *head)
{
    ...
    //kmem_cache_free(pm_cache, pm); // This leads to a kernel panic for some reason, so we do not free it here
}
...
static enum hrtimer_restart sample_fn(struct hrtimer *t)
{
	...
	for_each_process(p)
	{
		...
		if (pmu_supported && pm->insn_evt){
			// This needs refactoring as it can block and we are in an atomic context
...
static int __init pidmetrics_init(void)
{
	...
	// Setup the relay channel
	setup_relay();

	// Create the rhashtable cache
